...
int main(int argc, char *argv[]) {
	...
	if (verbose)
		printf("Files opened. Reading reference packet...\n");
	int nr;
	...
	while (read(inFile, &pSize, sizeof(pSize)) == sizeof(pSize)) {
		...
		if (testmode) {
			...
			for (int i = 0; i < sensorCount; ++i) {
				...
				memcpy(&sensorData, decompressed + 19 + i * 18,
						sizeof(sensorData));
...
void help() {
	...
	printf("outputfile\n");
	printf(
			"\tThe file to write the output to. Required with -v option. Without a specified filename, the output will be written to standard out.\n");
	printf("-t\n");
	printf(
			"\tTest mode. Use this flag when the recording is made when running the network test tools.\n");
	printf("-v\n");
	printf(
			"\tPrint (somewhat?) useful information to the standard out when processing the file. An output file has to be specified for this option to work.\n");
	printf("-------------\n");
	printf(
			"This program is part of a set of tools written for the monitoring system for the exoskeleton of Project MARCH\n");
	printf(
			"Written by Ruben Visser (r.visser at lunoct dot nl). Part of the bachelor thesis project of Jens Voortman, Vasco de Bruin and Ruben Visser.\n");
}
