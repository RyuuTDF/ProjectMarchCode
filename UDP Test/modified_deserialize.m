...
%   mydata = modified_deserialize(bytes);
%
        ...
        try
            ...
            for fn=fieldnames(conts)'
                try
                    ...
                    % a struct.
                    warn_once('modified_deserialize:restricted_access','No permission to set property %s in object of type %s.',fn{1},cls);
                end
            end
        catch
            v = conts;
            v.modified_deserialize_failed = ['could not construct class: ' cls];
        end
    end
end
end
